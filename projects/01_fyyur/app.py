...
def search_venues():
  ...
  search_venues = Venue.query.filter(Venue.name.like(f'%{search_term}%')).all()
...
def search_artists():
  ...
  search_artists = Artist.query.filter(Artist.name.like(f'%{search_term}%')).all()
...
def shows():
  ...
  venue_artist_shows = db.session.query(Show, Venue, Artist).join(Venue).join(Artist).all()
